...
class _CompoundInterestState extends State<CompoundInterest> {
  ...
  // Dropdown values
  String _timeType = 'Years';
  String _frequencyType = 'Annually';
  ...
  void _calculateCompoundInterest() {
    ...
    // Validate positive values and minimum time
    ...
    if (_timeType == 'Years' && time < 0.0833) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text('Time period must be at least 1 month (0.0833 years)'),
          backgroundColor: Colors.orange[600],
          behavior: SnackBarBehavior.floating,
          shape: RoundedRectangleBorder(
            borderRadius: BorderRadius.circular(10),
    ...
    if (_timeType == 'Months' && time < 1) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text('Time period must be at least 1 month'),
          backgroundColor: Colors.orange[600],
          behavior: SnackBarBehavior.floating,
          shape: RoundedRectangleBorder(
            borderRadius: BorderRadius.circular(10),
    ...
    double timeInYears = _timeType == 'Months' ? time / 12 : time;
    ...
    double totalAmount = principal * pow(1 + (annualRate / (compoundingFrequency * 100)), compoundingFrequency * timeInYears);
    double compoundInterest = totalAmount - principal;

    // Update state with results
    setState(() {
      _totalAmount = totalAmount.roundToDouble();
      _principalAmount = principal;
      _compoundInterest = compoundInterest.roundToDouble();
      _showResults = true;
    });
  }

  void _resetCompoundInterest() {
    setState(() {
      _principalController.clear();
      _rateController.clear();
      _timeController.clear();
      _timeType = 'Years';
      _frequencyType = 'Annually';
      _totalAmount = 0;
      _principalAmount = 0;
      _compoundInterest = 0;
      _showResults = false;
      ...
      children: [
        ...
        Container(
          ...
          child: DropdownButtonFormField<String>(
            ...
            items: <String>['Annually', 'Semi-Annually', 'Quarterly', 'Monthly']
                .map<DropdownMenuItem<String>>((String value) {
              return DropdownMenuItem<String>(
                value: value,
                child: Text(value),
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                _buildActionButtons(
                  onCalculate: _calculateCompoundInterest,
                  onReset: _resetCompoundInterest,
                ),
                const SizedBox(height: 30),
                if (_showResults) ...[
                  Text(
